...
# File names
kDataFileName <- "DATA/TIPTOPHHSBaselineDRC_DATA_WITH_NO_DUPS_2018-06-19_1600.csv"
...
district.data.cluster <- split(district.data, district.data[kDistrictClusterColumn])
...
names(district.data.cluster) %>%
  ...
  walk(function(cluster) {
    ...
    cluster.name <- district.clusters[district.clusters[kDistrictClusterColumn] == cluster,
...
map <- map %>% addLayersControl(overlayGroups = district.clusters[[kDistrictDlusterNameColumn]])
